...
// --------- CAN frame IDs ----------
static constexpr uint32_t CAN_ID_BSI_INFO = 0x260;   // Language/Units/Status (BSI -> Radio)
static constexpr uint32_t CAN_ID_OPTIONS = 0x361;    // Menu tab permissions (BSI -> Radio)
static constexpr uint32_t CAN_ID_SMEG_TIME = 0x39B;  // Time sync from SMEG (Radio -> BSI)
static constexpr uint32_t CAN_ID_RADIO_BTNS = 0x122; // Physical radio buttons (FMUX -> Radio)
static constexpr uint32_t CAN_ID_EMF_REMOTE = 0x3E5; // EMF Display remote commands
static constexpr uint32_t CAN_ID_BSI_TIME = 0x276;   // Time sync set to SMEG (BSI -> Radio)
static constexpr uint32_t CAN_ID_PARK_SENSORS = 0x0E1; // AAS / Parking sensors data (AEE2004)
static constexpr uint32_t CAN_ID_BSI_SLOW = 0x0F6;    // BSI Slow (Reverse, Economy, Brightness)
static constexpr uint32_t CAN_ID_HMI_CMD = 0x1A9;     // HMI Buttons from screen/steering
static constexpr uint32_t CAN_ID_CONF_SYNC = 0x15B;   // SMEG Configuration Sync request
...
static uint8_t last0F6[8] = {0, 0, 0, 0, 0, 0, 0, 0x65}; // Last BSI_SLOW for brightness

static bool forceParking = false;      // Manual parking emulator activation
static bool ecoBypass = false;         // Bypass car's Economy mode (prevent radio shutdown)
static bool ecoForce = false;          // Force radio into Economy Mode (remote power off)
  ...
  while (twai_receive_v2(src, &msg, 0) == ESP_OK && maxReads-- > 0) {
    ...
    if (isCarSide) {
      ...
      if (msg.identifier == CAN_ID_BSI_SLOW && msg.data_length_code >= 8) {
        memcpy(last0F6, msg.data, 8); // Save real frame baseline (brightness, etc.)
        carReverse = (msg.data[7] & 0x80) != 0;
        
        // Economy Mode Control (D5[0]): 1=Eco, 0=Normal
        if (ecoBypass) msg.data[5] &= ~0x01; // Force Normal to keep radio ON
        if (ecoForce)  msg.data[5] |= 0x01;  // Force Economy to turn radio OFF
        
        if (forceParking) continue; // Block real reverse to allow emulation
      }
      ...
      if (msg.identifier == CAN_ID_CONF_SYNC && msg.data_length_code >= 7) {
        if (msg.data[1] == 0x04) {
          ...
          if (nDRL != bsiDRL) { bsiDRL = nDRL; prefs.putBool("drl", bsiDRL); Serial.println("[NVS] DRL Upd"); }
          
          // AAS (Park Assist) button on screen triggers the emulator
          bool nPrk = (msg.data[4] & 0x80) != 0;
          if (nPrk != bsiParkAssist) { 
            bsiParkAssist = nPrk; 
            forceParking = nPrk; // Activate visualization immediately if user turns it ON
          ...
          if (nGrl != bsiGrilleLight) { bsiGrilleLight = nGrl; prefs.putBool("grille", bsiGrilleLight); Serial.println("[NVS] CAL Upd"); }
        }
      }
...
static void runBsiHeartbeat() {
  ...
  if (now - lastBsiHb < 500) return;
  lastBsiHb = now;
  if (!can1_handle) return;

  // --- 0x260: BSI_INFO (Status & Config) ---
  uint8_t d260[8] = {0};
  d260[0] = 0x80 | ((bsiLang & 0x1F) << 2);
  d260[1] = 0x04; // Default brightness/status
  if (!bsiIsCelsius) d260[1] |= (1 << 6);
  
  if (bsiDRL) d260[2] |= 0x02;
  if (bsiParkAssist) d260[4] |= 0x80;
  
  // Grille Light Status: Radio expects confirmation on Bit 2 (0x04)
  if (bsiGrilleLight) d260[6] |= 0x04; 
...
static void runParkingEmulation() {
  ...
  unsigned long now = millis();
  if (!(carReverse || forceParking) || (now - lastPrk < 100)) return;
  lastPrk = now;
  if (!can1_handle) return;
