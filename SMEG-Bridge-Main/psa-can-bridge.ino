...
// --------- CAN frame IDs ----------
static constexpr uint32_t CAN_ID_BSI_INFO = 0x260;   // Language/Units/Status
static constexpr uint32_t CAN_ID_OPTIONS = 0x361;    // Menu tab permissions
static constexpr uint32_t CAN_ID_SMEG_TIME = 0x39B;  // Time sync from SMEG
static constexpr uint32_t CAN_ID_RADIO_BTNS = 0x122; // Radio buttons
static constexpr uint32_t CAN_ID_EMF_REMOTE = 0x3E5; // EMF Display remote
static constexpr uint32_t CAN_ID_BSI_TIME = 0x276;   // Time sync set to SMEG
static constexpr uint32_t CAN_ID_PARK_SENSORS = 0x0E1;
static constexpr uint32_t CAN_ID_BSI_SLOW = 0x0F6;
static constexpr uint32_t CAN_ID_HMI_CMD = 0x1A9;
static constexpr uint32_t CAN_ID_CONF_SYNC = 0x15B;

// --------- State & Emulation ----------
static unsigned long lastCan0Rx = 0;
static unsigned long lastCan1Rx = 0;
static uint32_t canDroppedFrames = 0;

static uint8_t bsiLang = 14;        // Default RU
static bool bsiIsCelsius = true;    // Default °C
static bool bsiDRL = true;          // Status for 0x260 D2[1]
static bool bsiParkAssist = true;   // Status for 0x260 D4[7]
static bool bsiGrilleLight = false; // Status for 0x260 D6[2]
static bool carReverse = false;
static bool lastBtn1A9 = false;
static Preferences prefs;

static uint8_t mod122[8] = {0};
static uint8_t last122[8] = {0};
static uint8_t last0F6[8] = {0, 0, 0, 0, 0, 0, 0, 0x65};

static bool forceParking = false;
static uint8_t parkDist[6] = {7, 7, 7, 7, 7, 7};
  ...
  while (twai_receive_v2(src, &msg, 0) == ESP_OK && maxReads-- > 0) {
    ...
    if (isCarSide) {
      ...
      if (msg.identifier == CAN_ID_BSI_SLOW && msg.data_length_code >= 8) {
        memcpy(last0F6, msg.data, 8); // Save for later spoofing
        carReverse = (msg.data[7] & 0x80) != 0;
        if (forceParking) continue; // Block real reverse when emulating
      }
...
static void runBsiHeartbeat() {
  ...
  if (bsiParkAssist)
    d260[4] |= 0x80;
  // Grille Light Status: Radio expects it on Bit 2 (0x04) to show the tick
  if (bsiGrilleLight)
    d260[6] |= 0x04; 
